...
# You have no "not reported", so don't need that

tidy <- tidy %>%
  filter(!is.na(ann_arbor_b_symptoms)) # changed from "pheno" to "tidy"
```
...
table(colnames(exprs_mat) == rownames(pheno_mat)) 

# check
pheno_mat <- pheno_mat[order(rownames(pheno_mat), colnames(exprs_mat)), ] 
